...
Where:
- "improved_job_description" is the full, cleaned and structured description tailored for internal recruiter screening.
- "high_level_summary" briefly explains the main changes and reasoning, especially how it now better supports CV screening.
- "detailed_changes" is a list of bullet-style strings, each describing a specific change and why it was made.
...
overall_fit_rating MUST be an integer from 1 to 10.
1 = terrible fit (almost no requirements met).
10 = perfect fit (all MUST-HAVES + most NICE-TO-HAVES).
Use the full 1–10 range across many candidates, not only 3–5.
A higher number always means a better fit.
...
def evaluate_single_resume(llm, fixed_jd, resume_text, resume_link):
    data = llm_json(
        llm,
        RECRUITER_PROMPT,
    ...
    sr = ScreeningResult(
        candidate_strengths=data["candidate_strengths"],
        candidate_weaknesses=data["candidate_weaknesses"],
        risk_factor=RiskReward(**data["risk_factor"]),
        reward_factor=RiskReward(**data["reward_factor"]),
        overall_fit_rating=int(data["overall_fit_rating"]),
        justification_for_rating=data["justification_for_rating"],
        Date=time.strftime("%Y-%m-%d %I:%M %p"),
        Resume=resume_link,
        Full_Resume=resume_text  # Store full resume, not truncated
    )
...
def results_to_dataframe(results: List[ScreeningResult]):
    rows = []
    for r in results:
        # Store full resume in the dataframe (Excel can handle long text)
        rows.append({
            "Date": r.Date,
            ...
            "Email": r.Email,
            "Strengths": "\n\n".join(r.candidate_strengths),
            "Weaknesses": "\n\n".join(r.candidate_weaknesses),
            ...
            "Full Resume": r.Full_Resume,  # Full resume text
        })
    return pd.DataFrame(rows)

def save_excels(df, out_dir, threshold):
    ensure_dir(out_dir)
    high_scoring_dir = os.path.join(out_dir, "High Scoring Candidates")
    all_results_dir = os.path.join(out_dir, "Resume Screening Results")
    ensure_dir(high_scoring_dir)
    ensure_dir(all_results_dir)
    
    all_path = os.path.join(all_results_dir, "Resume_Screening_Results.xlsx")
    high_path = os.path.join(high_scoring_dir, "High_Scoring_Candidates.xlsx")

    df.to_excel(all_path, index=False)
    df[df["Overall Fit"] >= threshold].to_excel(high_path, index=False)

    return all_path, high_path

def save_jd_pdf(fixed_jd: str, improved_jd_dir: str) -> str:
    """Save the improved JD as a simple, readable PDF with basic wrapping."""
    ...
    c.drawString(x_margin, y, "Improved Job Description")
    y -= 24

    c.setFont("Helvetica", 11)
    for paragraph in fixed_jd.splitlines():
...
# Page 0.5: Filtering Process
    ...
    try:
        ...
        raw_improved = jd_fix_result["improved_job_description"]
        ...
        # Save improved JD
        improved_jd_pdf_path = save_jd_pdf(fixed_jd, cfg.Improved_JD_dir)
        ...
        uploaded_resumes = []
        for r in resumes:
            uploaded_resumes.append({"name": r["name"], "text": r["text"], "link": ""})
        ...
        def process_item(item):
            return evaluate_single_resume(llm, fixed_jd, item["text"], item["link"])
